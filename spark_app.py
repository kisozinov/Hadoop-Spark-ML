...
def main(num_nodes: int, optimize: bool):
    ...
    for _ in range(20): # num experiments
        cumulative_times = []
        total_mems = []
        total_time = 0
        ...
        all_times.append(cumulative_times)
        all_mems.append(total_mems)

    cumulative_times = list(np.mean(np.array(all_times), axis=0))
    total_mems = list(np.mean(np.array(all_mems), axis=0))
